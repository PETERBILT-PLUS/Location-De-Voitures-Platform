...
const login = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const findUser = yield user_model_js_1.default.findOne({ email: email });
        if (!findUser)
            return res.status(404).json({ success: false, message: "Some of the information or all the informatins provided is incorrect. Please check and try again." });
        ...
        res.status(200).json({ success: true, message: "User Sign In Succesful" });
    }
    catch (error) {
        res.status(500).json({ success: false, message: "Internal Server Error" });
    }
